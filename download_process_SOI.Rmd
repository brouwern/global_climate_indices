---
title: "Southern Oscillation Index Download and Prep"
...
library(ggplot2)
library(rprojroot)  #working directory management
```


## Set up root diretory
```{r}
...
#Extract month
SOI.all.yrs$mo <- gsub("^([12][1-90][1-90][1-90])(.*)",
                 "\\2",SOI.all.yrs$Date)
...
#create filename
file. <- paste("climate_index_SOI_", year, ".csv", sep = "")
dir.  <- "./data/data_out"
file. <- paste(dir.,file.,sep = "/")

write.csv(SOI.all.yrs, file = file.)
```
...
winter.means <- apply(SOI.months.wide[,c("01","02","03")],1,mean)
spring.means <- apply(SOI.months.wide[,c("04","05","06")],1,mean)
summer.means <- apply(SOI.months.wide[,c("07","08","09")],1,mean)
...
```{r}
#Determine year range
year <- paste(range(SOI.seasonal.means$yr),collapse = "_")

#create filename
file. <- paste("climate_index_SOI_seasonal_summary", year, ".csv", sep = "")
file. <- paste(dir.,file., sep = "/")
write.csv(SOI.seasonal.means, file = file.)
```
...
#create filename
file. <- paste("climate_index_SOI_ann_summary", year, ".csv", sep = "")
file. <- paste(dir.,file., sep = "/")
write.csv(SOI.ann.summary, file = file.)
